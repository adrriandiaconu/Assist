...
@Serializable data class BackupDocument(val id:Long,val patientId:Long,val title:String,val category:String,val localPath:String,val fileType:String)
...
class BackupManager(private val context: Context, private val db: AppDatabase) {
    suspend fun export(treeUri: Uri): Boolean {
        ...
        val visits = db.visitDao().byPatient(-1).first().filter { false } // placeholder empty aggregate
        val payload = FullBackup(
            patients.map { BackupPatient(it.id,it.fullName,it.dateOfBirth,it.notes,it.relationType) },
            docs.map { BackupDocument(it.id,it.patientId,it.title,it.category.name,it.localPath,it.fileType) },
        ...
        context.contentResolver.openOutputStream(metadata.uri)?.use { it.write(Json { prettyPrint = true }.encodeToString(payload).toByteArray()) }
        docs.forEach { doc ->
