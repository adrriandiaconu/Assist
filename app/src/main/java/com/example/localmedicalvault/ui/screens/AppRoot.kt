...
@Composable fun AppRoot(vm: VaultVM = viewModel()) {
    ...
    when(val s=screen) {
        Screen.Home -> Scaffold(floatingActionButton = { FloatingActionButton(onClick={screen=Screen.PatientForm(null)}){Text("+")}}) { p->
            Column(Modifier.padding(p).padding(16.dp)) { Text("Family Medical Vault", style=MaterialTheme.typography.headlineSmall); Row{ Button(onClick={screen=Screen.DocForm(null)}){Text("Add document")}; Spacer(Modifier.width(8.dp)); Button(onClick={screen=Screen.Search}){Text("Search")}; Spacer(Modifier.width(8.dp)); Button(onClick={screen=Screen.Settings}){Text("Settings")}}
                if (patients.isEmpty()) Text("No patients yet") else LazyColumn { items(patients){ pat -> Text("${pat.fullName} (${pat.relationType})", modifier=Modifier.fillMaxWidth().clickable { screen=Screen.PatientDetail(pat.id) }.padding(10.dp)) } }
            }
        }
...
@Composable private fun PatientFormScreen(id:Long?, vm:VaultVM, onDone:()->Unit){ var n by remember{mutableStateOf("")}; var dob by remember{mutableStateOf("")}; var notes by remember{mutableStateOf("")}; var rel by remember{mutableStateOf("child")}; Column(Modifier.padding(16.dp)){ Text("Patient"); OutlinedTextField(n,{n=it},label={Text("Full name")}); OutlinedTextField(dob,{dob=it},label={Text("Date of birth (optional)")}); OutlinedTextField(notes,{notes=it},label={Text("Notes")}); OutlinedTextField(rel,{rel=it},label={Text("Relation")}); Button(onClick={ vm.addOrUpdatePatient(id,n,dob,notes,rel); onDone()}){Text("Save")}}
}
...
@Composable private fun DocumentFormScreen(defaultPatientId:Long?, vm:VaultVM, patients:List<PatientEntity>, onDone:()->Unit){ val context=LocalContext.current; var patientId by remember{mutableStateOf(defaultPatientId)}; var title by remember{mutableStateOf("")}; var date by remember{mutableStateOf("")}; var doctor by remember{mutableStateOf("")}; var clinic by remember{mutableStateOf("")}; var tags by remember{mutableStateOf("")}; var notes by remember{mutableStateOf("")}; var cat by remember{mutableStateOf(DocumentCategory.OTHER)}; var selected by remember{mutableStateOf<Uri?>(null)}; var name by remember{mutableStateOf<String?>(null)}; val picker = rememberLauncherForActivityResult(ActivityResultContracts.OpenDocument()){ uri -> if(uri==null){Toast.makeText(context,"File selection cancelled",Toast.LENGTH_SHORT).show();return@rememberLauncherForActivityResult}; selected=uri; context.contentResolver.takePersistableUriPermission(uri, Intent.FLAG_GRANT_READ_URI_PERMISSION); name=uri.lastPathSegment }
 Column(Modifier.padding(16.dp)){ Text("Add document"); if(patients.isEmpty()) Text("No patients yet") else patients.forEach { p -> Text((if(patientId==p.id) "• " else "") + p.fullName, Modifier.clickable { patientId = p.id }.padding(2.dp)) }
 Button(onClick={picker.launch(arrayOf("image/*","application/pdf"))}){Text("Select file")}; Text(name?:"No file selected"); OutlinedTextField(title,{title=it},label={Text("Title")}); OutlinedTextField(date,{date=it},label={Text("Date")}); OutlinedTextField(doctor,{doctor=it},label={Text("Doctor")}); OutlinedTextField(clinic,{clinic=it},label={Text("Clinic")}); OutlinedTextField(tags,{tags=it},label={Text("Category/Tags")}); OutlinedTextField(notes,{notes=it},label={Text("Notes")}); Button(onClick={vm.addDocument(patientId,title,cat,date,doctor,clinic,tags,notes,selected,name){ok,msg -> Toast.makeText(context,msg,Toast.LENGTH_SHORT).show(); if(ok) onDone() }}){Text("Save")}}
}
@Composable private fun DocumentDetailScreen(id:Long, vm:VaultVM, onBack:()->Unit){ val docs by vm.allDocs.collectAsState(); val context=LocalContext.current; val d=docs.firstOrNull{it.id==id}; var ask by remember{mutableStateOf(false)}; var err by remember{mutableStateOf<String?>(null)}; if(d==null){Text("Not found"); return}; Column(Modifier.padding(16.dp)){ Text(d.title); Text("${d.category} / ${d.documentDate}"); err?.let { Text(it, color = MaterialTheme.colorScheme.error) }; Button(onClick={ val f=File(d.localPath); if(!f.exists()){err="File missing from local storage"; return@Button}; val uri=FileProvider.getUriForFile(context, context.packageName+".fileprovider", f); val intent=Intent(Intent.ACTION_VIEW).apply { setDataAndType(uri, if(d.fileType=="pdf")"application/pdf" else "image/*"); addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION) }; runCatching{ context.startActivity(intent) }.onFailure{err="No app available to open this file"} }){Text("Open Document")}; Button(onClick={ask=true}){Text("Delete")}; Button(onClick=onBack){Text("Back")}}
...
@Composable private fun SettingsScreen(vm:VaultVM, onBack:()->Unit){ var msg by remember{mutableStateOf("")}; val picker = rememberLauncherForActivityResult(ActivityResultContracts.OpenDocumentTree()){ uri -> if(uri!=null) vm.exportBackup(uri){ok-> msg= if(ok)"Backup exported" else "Backup failed. Check folder permissions and storage." } else msg="Export cancelled" }
 Column(Modifier.padding(16.dp)){ Text("Settings"); Button(onClick={picker.launch(null)}){Text("Export backup")}; Text(msg); Button(onClick=onBack){Text("Back")}}
}
