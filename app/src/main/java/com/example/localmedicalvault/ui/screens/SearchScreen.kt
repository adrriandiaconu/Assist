...
@Composable fun SearchScreen(padding: PaddingValues, vm: VaultVM, docs: List<MedicalDocumentEntity>, patients: List<PatientEntity>, onOpen:(Long)->Unit){
    ...
    Column(Modifier.padding(padding).padding(16.dp), verticalArrangement=Arrangement.spacedBy(10.dp)){ Text("Search", style=MaterialTheme.typography.headlineSmall); Text("Find documents fast", color=MaterialTheme.colorScheme.onSurfaceVariant)
        ...
        LazyColumn(verticalArrangement=Arrangement.spacedBy(8.dp)){ items(results){ d -> DocumentRowCard(d, patients.firstOrNull { it.id == d.patientId }?.fullName ?: "Unknown member", onOpen) } }
    }
}
